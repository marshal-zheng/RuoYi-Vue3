  ...
  <div class="w-full">
    ...
    <div class="mb-4">
      <vxe-table
        ...
        class="text-sm"
      >
        <vxe-column field="label1" title="" width="100" align="right" class-name="!bg-gray-50 !font-medium" />
        <vxe-column field="value1" title="" min-width="150" :edit-render="configEditRender('value1')" />
        <vxe-column field="label2" title="" width="100" align="right" class-name="!bg-gray-50 !font-medium" />
        <vxe-column field="value2" title="" min-width="150" :edit-render="configEditRender('value2')" />
        <vxe-column field="label3" title="" width="100" align="right" class-name="!bg-gray-50 !font-medium" />
        <vxe-column field="value3" title="" min-width="150" :edit-render="configEditRender('value3')" />
        <vxe-column field="label4" title="" width="100" align="right" class-name="!bg-gray-50 !font-medium" />
        <vxe-column field="value4" title="" min-width="150" :edit-render="configEditRender('value4')" />
      </vxe-table>
    </div>
    ...
    <div>
      <vxe-toolbar>
        <template #buttons>
          ...
          <ZxButton icon="Delete" type="danger" plain style="margin-left:10px" @click="removeSelectRow">删除</ZxButton>
        </template>
      </vxe-toolbar>
      ...
      >
        ...
        <vxe-column field="序号" title="序号" width="80" align="center" :edit-render="{ name: 'input' }" />
        <vxe-column field="信息名称" title="信息名称" width="150" :edit-render="{ name: 'input' }" />
        <vxe-column field="字节数" title="字节数" width="100" align="center" :edit-render="{ name: 'input' }" />
        <vxe-column field="字号" title="字号" width="100" align="center" :edit-render="{ name: 'input' }" />
        <vxe-column field="值域及含义" title="值域及含义" width="200" :edit-render="{ name: 'input' }" />
        <vxe-column field="量纲" title="量纲" width="100" :edit-render="{ name: 'input' }" />
        <vxe-column field="数据类型" title="数据类型" width="120" :edit-render="{ name: 'input' }" />
        ...
        <vxe-column title="操作" width="100" align="center" fixed="right">
          <template #default="{ rowIndex }">
            <zx-button type="danger" icon="Delete" plain  @click="removeRow(rowIndex)">删除</zx-button>
          </template>
        </vxe-column>
      </vxe-table>
    </div>
...
const configTableData = computed(() => [
  ...
  },
  {
    ...
    label2: '子地址',
    value2: localConfig.subAddress,
    label3: '帧长度',
    value3: `${localConfig.frameLength} word`,
    label4: '错误处理',
...
const configEditRender = (field: string) => {
  ...
  const isReadonly = (field === 'value1' && row.label1 === '发送方') || 
...
const insertRow = async () => {
  // 自动计算下一个序号
  const nextSeqNo = localConfig.fields.length > 0 
